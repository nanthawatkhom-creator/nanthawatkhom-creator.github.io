class SoundManager {
  ...
  public playSystemBreach() {
    if (this.isMuted) return;
    try {
      this.initContext();
      if (!this.ctx) return;
      const now = this.ctx.currentTime;
      // Heavy dramatic siren chord for immediate game over
      const freqs = [550, 440, 330, 220, 110];
  ...
  public playRadioDispatch() {
    ...
    try {
      ...
      // 2-tone walkie-talkie / radio squelch chirp
      const osc1 = this.ctx.createOscillator();
      const osc2 = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
      ...
      osc1.frequency.setValueAtTime(1174.66, now + 0.08); // A5 -> D6
  ...
  public playRestoredWave() {
    ...
    try {
      ...
      // Lush digital chime chord for complete system recovery
